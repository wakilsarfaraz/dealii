                                  ...
                                  const unsigned int /*component*/) const
{
  ...
  return 2*M_PI*M_PI*sin(numbers::PI*x)*cos(numbers::PI*y);
   }
   else

   {double z = p(2);

   return 3*M_PI*M_PI*sin(M_PI*x)*cos(M_PI*y)*sin(M_PI*z);
   }
}
                                   ...
                                   const unsigned int /*component*/) const
{
  ...
  if (dim==2)
    ...
    {
      double z = p(2);
      return sin(M_PI*x)*cos(M_PI*y)*sin(M_PI*z);
    }
}
                                   ...
                                   const unsigned int /*component*/) const
{
  ...
  if (dim==2)
    {
      return_value[0] = M_PI*cos(M_PI*x)*cos(M_PI*y);
      return_value[1] = -M_PI*sin(M_PI*x)*sin(M_PI*y);
    }
  else
    {  double z = p(2);
      return_value[0] = M_PI*cos(M_PI*x)*cos(M_PI*y)*sin(M_PI*z);
      return_value[1] = -M_PI*sin(M_PI*x)*sin(M_PI*y)*sin(M_PI*z);
      return_value[2] = M_PI*sin(M_PI*x)*cos(M_PI*y)*cos(M_PI*z);
    }
...
void Step4<dim>::run ()
{
  ...
  make_grid();
    ...
    for (  cycle = 0;cycle < num_refinements; ++cycle)
    {
