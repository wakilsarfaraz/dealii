                                  ...
                                  const unsigned int /*component*/) const
{
  ...
  return 0.25*sin(3*theta)*((M_PI*M_PI*r*r+20)*cos(M_PI*r/2)+10*M_PI*r*sin(M_PI*r/2));
}
                                   ...
                                   const unsigned int /*component*/) const
{
  ...
  double theta = atan2(y,x);
  if (dim==2)
    return r*r*sin(3*theta)*cos(M_PI*r/2);
  else
    return 0.0; // TODO
}
                                   ...
                                   const unsigned int /*component*/) const
{
  ...
  if (dim==2)
    {
      return_value[0] = 0.5*(-6*y*cos(0.5*M_PI*r)*cos(3*theta)+4*x*cos(0.5*M_PI*r)*sin(3*theta)
                              -M_PI*x*r*sin(0.5*M_PI*r)*sin(3*theta));
      return_value[1] = 0.5*(6*x*cos(0.5*M_PI*r)*cos(3*theta)+4*y*cos(0.5*M_PI*r)*sin(3*theta)
                             -M_PI*y*r*sin(0.5*M_PI*r)*sin(3*theta));
    }
  else
    {
      // TODO
...
void Step4<dim>::assemble_system ()
{
  ...
  VectorTools::interpolate_boundary_values (mapping,
                                            dof_handler,
                                            0,
                                            /*SolutionValues<dim>()*/ZeroFunction<dim>(),
                                            boundary_values);
